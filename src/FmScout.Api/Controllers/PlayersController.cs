...
public sealed class PlayersController(FmScoutDbContext db) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<PlayerDto>> GetById(Guid id,CancellationToken ct)
    {
        var x=await db.Players.AsNoTracking().FirstOrDefaultAsync(p=>p.Id==id,ct); if(x is null)return NotFound();
        return Ok(new PlayerDto(x.Id,x.Name,x.Slug,x.Age,x.Position,x.Club,x.Nation,x.CurrentAbility,x.PotentialAbility,x.MarketValue,x.IsWonderkid,x.IsHiddenGem));
    }
    [HttpPost]
    public async Task<ActionResult<PlayerDto>> Create(CreatePlayerRequest r,CancellationToken ct)
    {
        ...
        return CreatedAtAction(nameof(GetById),new{id=p.Id},new PlayerDto(p.Id,p.Name,p.Slug,p.Age,p.Position,p.Club,p.Nation,p.CurrentAbility,p.PotentialAbility,p.MarketValue,p.IsWonderkid,p.IsHiddenGem));
    }
}
